...
function statusCol (statusText) {
  const status = _.trim(statusText.slice(0, statusText.match(/\(/).index))
  const time = _.trim(statusText.slice(statusText.match(/\)/).index + 1))
  return `${status} ${time}`
}

function print (stdout) {
  if (stdout === '') {
    return `${chalk.magenta('No one containers are started.')}
